...
text = Path("Jellyfin.Plugin.ArtistTagShelf.csproj").read_text()
...
cp "bin/Release/net9.0/Jellyfin.Plugin.ArtistTagShelf.dll" "$stage/"
cp meta.json "$stage/"
cp backdrop.svg "$stage/"

mkdir -p dist
zip_path="$root/dist/ArtistTagShelf_${version}.zip"
rm -f "$zip_path"
...
with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
    for name in ("Jellyfin.Plugin.ArtistTagShelf.dll", "meta.json", "backdrop.svg"):
...
source_url="https://github.com/TidBits16/ArtistTagShelf/releases/download/v${version}/ArtistTagShelf_${version}.zip"
