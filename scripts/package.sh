...
text = Path("Jellyfin.Plugin.ArtistFin.csproj").read_text()
...
cp "bin/Release/net9.0/Jellyfin.Plugin.ArtistFin.dll" "$stage/"
cp meta.json "$stage/"
cp backdrop.svg "$stage/"

mkdir -p dist
zip_path="$root/dist/artistfin_${version}.zip"
rm -f "$zip_path"
...
with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
    for name in ("Jellyfin.Plugin.ArtistFin.dll", "meta.json", "backdrop.svg"):
...
source_url="https://github.com/TidBits16/ArtistFin/releases/download/v${version}/artistfin_${version}.zip"
